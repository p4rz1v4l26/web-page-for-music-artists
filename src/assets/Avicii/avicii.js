const youtubeApi =
  "https://youtube-v31.p.rapidapi.com/search?channelId=UCPHjpfnnGklkRBBTd0k6aHg&part=snippet%2Cid&order=viewcount&maxResults=10";
const spotifyApi =
  "https://spotify23.p.rapidapi.com/artist_albums/?id=1vCWHaC5f2uS3yhpwWbIA6&offset=0&limit=8";
const singlesApi =
  "https://spotify23.p.rapidapi.com/artist_singles/?id=1vCWHaC5f2uS3yhpwWbIA6&offset=0&limit=20";
...
const spotifyOptions = {
  method: "GET",
  headers: {
    "X-RapidAPI-Key": "60e8ca4ddemsh4de1db7ef8716bfp1d6ae7jsnb7f74703b2a4",
  ...
  try {
    ...
    let view = `
        ...
        <a href="https://youtube.com/watch?v=${video.id.videoId}" target="_blank">
            <div class="group relative bg-green-300">
                <div
...
async function getAlbums() {
  try {
    ...
    if (
      !albums.data.artist ||
      !albums.data.artist.discography ||
      !albums.data.artist.discography.albums
    ) {
    ...
    let view = `
        ${albums.data.artist.discography.albums.items
          .map(
            (album) => `
        <div class="bg-green-300 rounded-lg p-4">
        <a href="${album.releases.items[0].sharingInfo.shareUrl}" target="_blank">
            <img class="w-full mb-4 rounded-lg bg-red-900" src="${album.releases.items[0].coverArt.sources[0].url}"
...
async function getSingles() {
  try {
    ...
    if (
      !singles.data.artist ||
      !singles.data.artist.discography ||
      !singles.data.artist.discography.singles
    ) {
